...
class GaussCRF(nn.Module):
    ...
    def forward(self, unary, img, num_iter=5):
        ...
        col_feats = self.create_colour_feats(
            img, sdims=self.col_sdims, schan=self.col_schan, bias=conf["col_feats"]["use_bias"], bs=bs
        )
    ...
    def _create_convolutional_filters(self, features):
        ...
        if self.blur > 1:
            ...
            features = torch.nn.functional.avg_pool2d(
                features, kernel_size=self.blur, padding=(pad_0, pad_1), count_include_pad=False
            )
    ...
    def _compute_gaussian(self, input, gaussian, norm=None):
        ...
        if self.blur > 1:
            ...
            input = torch.nn.functional.avg_pool2d(
                input, kernel_size=self.blur, padding=(pad_0, pad_1), count_include_pad=False
            )
            npixels = [math.ceil(self.npixels[0] / self.blur), math.ceil(self.npixels[1] / self.blur)]
        ...
        if self.pyinn:
            ...
            input_unfold = input_unfold.view(
                bs, num_channels, self.filter_size, self.filter_size, npixels[0], npixels[1]
            )
